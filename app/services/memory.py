...
Design choices:
  - Markdown for profiles: LLM-readable AND LLM-writable, zero impedance mismatch
  - JSONL for conversations: append-only, easy to tail, no parse-whole-file
  - JSON for tasks: small, mutable, read-modify-write is fine at this scale
"""
...
from app.config import MEMORY_CONTEXT_LIMIT, MEMORY_RECENT_LIMIT, USER_DATA_DIR
...
class UserMemory:
    ...
    def formatted_recent(self) -> str:
        ...
        Includes the most recent messages for conversational continuity.
        """
        lines = []
...
async def load_memory(user_id: str) -> UserMemory:
    ...
    Returns a UserMemory with profile text, recent conversations, and active tasks.
    Missing files are handled gracefully — new users get sensible defaults.
    """
    ...
    # Recent conversations (last N from JSONL)
...
async def _load_conversations(user_dir: Path) -> list[dict]:
    """Load conversation history from JSONL. Returns list of message dicts."""
...
async def append_conversation(
    ...
    Args:
        ...
        direction: "in" for user messages, "out" for Goon responses
        text: Message content
        metadata: Optional extra fields (e.g., message_sid, call_id)
    """
    user_dir = _user_dir(user_id)
...
async def update_memory(
    ...
    This is the main entry point called after each orchestrator response.

    Args:
        user_id: Phone number or user identifier
        user_message: What the user texted
        response_text: What Goon responded
        memory_updates: List of dicts with "category" and "content" keys,
                       generated by the LLM's update_memory tool calls
    """
    # Append both sides of the conversation
    await append_conversation(user_id, "in", user_message)
    await append_conversation(user_id, "out", response_text)

    # Apply profile updates if the LLM flagged any
    if memory_updates:
        user_dir = _user_dir(user_id)
        user_dir.mkdir(parents=True, exist_ok=True)
...
async def apply_profile_updates(user_dir: Path, updates: list[dict]) -> None:
    ...
    The LLM reads the current profile and the updates, then produces a
    complete updated profile that preserves existing info and integrates
    new facts into the appropriate sections.

    Args:
        user_dir: Path to user's data directory
        updates: List of dicts with "category" and "content" keys
    """
    import anthropic
...
async def save_profile(user_id: str, profile_text: str) -> None:
    """Directly write a profile.md for a user. Used for bootstrapping."""
    user_dir = _user_dir(user_id)
    user_dir.mkdir(parents=True, exist_ok=True)
...
async def add_task(user_id: str, description: str, trigger: str = "") -> dict:
    """Add a task to the user's task list. Returns the new task dict."""
...
async def get_business_conversations(
    ...
    Used to inject business-specific context into the LLM prompt,
    even if those messages aren't in the most recent N.
    """
